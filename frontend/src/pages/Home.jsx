...
// Backend URL from environment variable or fallback for local development
const BACKEND_URL =
...
export default function Home() {
  ...
  const handleFileUpload = async (file) => {
    ...
    try {
      ...
      const response = await axios.post(`${BACKEND_URL}/api/extract-skills`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      ...
      setError(err.response?.data?.detail || "Failed to extract skills. Please try again.");
    } finally {
      setLoading(false);
    }
  };
